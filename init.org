...
#+begin_src emacs-lisp
(elpaca (doom-lib :host github
                  :repo "doomemacs/doomemacs"
                  :branch "master"
                  :files ("lisp/doom-lib.el")))
...
(use-package evil-escape
  :after evil
  :init
  (keymap-set evil-insert-state-map "C-g" #'evil-escape)
...
(use-package cape
  ...
  (add-hook! (prog-mode conf-mode comint-mode minibuffer-setup eshell-mode)
...
#+begin_src emacs-lisp
(defadvice! cape--dabbrev-list-ad (input)
  "Find all Dabbrev expansions for INPUT."
  :override #'cape--dabbrev-list
  (cape--silent
...
(use-package tabspaces
  ...
  (defun tabspaces-open-or-create-project ()
    (interactive)
    (tabspaces-open-or-create-project-and-workspace (funcall project-prompter) current-prefix-arg))
  (tabspaces-mode 1)
  :bind (:map project-prefix-map ("p" . tabspaces-open-or-create-project))
  :bind ("M-z" . tab-bar-mode)
  :config
...
(add-hook! (LaTeX-mode latex-mode org-mode) #'math-prettify-activate)
#+end_src
...
*** Skip 'SUBTREE'
#+begin_src emacs-lisp
(defadvice! org-cycle-internal-local-ad (f)
  :around #'org-cycle-internal-local
...
(after! (eglot tex-mode)
  ...
  (defun eglot-texlab-build ()
    ...
    (jsonrpc-async-request
     ...
     :success-fn (lambda (r) (message "Error %s" r))
     :deferred t))
...
(use-package mamimo
  :ensure (:repo "~/.config/emacs-new/lisp/lib/mamimo"
           :files ("*.el" "snippets"))
  :commands (mamimo-mode
             mamimo-yas-mode
             mamimo-smartkeys-mode)
  :config
  (add-hook 'mamimo-mode-hook 'evil-tex-mode))
#+end_src
